...
public class BarScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // ボールを生成
        Vector3 ballShootPos = new Vector3(transform.position.x, transform.position.y + 1.0f, transform.position.z);    // 位置決め
        newBall = Instantiate(ballPrefab, ballShootPos, Quaternion.identity);   // 球を生成
        newBall.tag = "Ball";   // クローンにタグBallを追加
        shoot = false;          // 射出フラグをリセット
    ...
    void Update()
    {
        ...
        float moveY = Input.GetAxisRaw("Vertical") * moveSpeed * Time.deltaTime;   // 垂直方向の移動量
        // 移動
